...
// Function to delete data from the graphData
function deleteData(year, id) {
  try {
    const graphData = readGraphData();

    if (graphData.data[year]) {
      graphData.data[year] = graphData.data[year].filter(item => item.id !== id);

      // Update the graph.json file with the updated data
      const fs = require('fs');
      fs.writeFileSync('./graph.json', JSON.stringify(graphData, null, 2));
    }
  } catch (error) {
...
app.delete('/delete-data/:year/:id', (request, response) => {
